...
public class Main
{
    ...
    public static void insertar(String linea,Carpeta papa,int n){
        Carpeta padre = papa.ultimaC(); //es la ultima carpeta agregada a algún lado         
        ...
        if(n == 0 && unaCarpeta){ //esta en carp Principal y es una carpeta??
            ...
            System.out.println("$[MOD]$ Se agregó la carpeta ["+carp.nombre+"] a la carpeta ["+papa.ultimaC().nombre+"]");
        }else if(n == 0 && !unaCarpeta){ //esta en carp Principal y es un fichero??
            papa.ultimaC().insertarF(linea);
            System.out.println("$[MOD]$ Se agregó el fichero ["+linea+"] a la carpeta ["+papa.ultimaC().nombre+"]");
        }else{
        ...
        while( (linea = buf.readLine()) != null ){
            ...
            if(linea.charAt(0) == '+'){ 
                ...
                if(cadena[1].indexOf('.') != -1){ //busca si hay un punto en el nombre
                    ...
                    System.out.println("Se incertó el fichero:"+cadena[1]);
                }else{
                    Carpeta carp = new Carpeta(cadena[1],papa);
                    papa.insertarC(carp);
                    System.out.println("Se incertó la carpeta:"+cadena[1]);
                    carpAct++;
                    actual = papa.thisCarpeta(carpAct).nombre;
                }
                System.out.println("\tEstoy en la carpeta: "+actual);
            }else if(linea.charAt(0) == '�'){//Esta condicion es para crear los demas ficheros y carpetas
                ...
                System.out.println("# de espacios: "+cont+"\tSe pondrá en carpeta: "+numCarpeta);
                String[] cadena = linea.split("]  ");
